	...
	return (
		<nav className={`h-14 fixed w-full z-[999] ${pathname === '/' ? null : 'bg-white'}`}>
			<ul className='max-w-7xl mx-auto flex gap-3 h-full items-center'>
				...
				<li>
					{email ? (
						<button className='border border-black px-2 py-1 rounded-full hover:border-primary hover:text-white hover:bg-primary hover:px-4 transition-all'>Logout</button>
					) : (
